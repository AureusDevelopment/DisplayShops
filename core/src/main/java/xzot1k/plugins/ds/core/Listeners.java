...
public class Listeners implements Listener {
    ...
    private void handlePickup(Cancellable e, Item localItem, Entity entity) {
        if (localItem.hasMetadata("shop-owner")) {
            for (MetadataValue value : localItem.getMetadata("shop-owner"))
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        final boolean isOffhandVersion = Ref.isNewerThan(8) || Ref.isMajorNewerThan(26);
        if (isOffhandVersion && (Ref.isNewerThan(11)|| Ref.isMajorNewerThan(26)) && e.getHand() != EquipmentSlot.HAND)
            return;
        ...
        if (dataPack.isInSelectionMode()) {
            if (isOffhandVersion && (Ref.isNewerThan(12)|| Ref.isMajorNewerThan(26)) && e.getHand() == EquipmentSlot.OFF_HAND) {
                e.setCancelled(true);
                return;
            }
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (e.getClickedBlock() != null) {
                ...
                if (Ref.isNewerThan(13)|| Ref.isMajorNewerThan(26)) {
    ...
    public void onEdit(ShopEditEvent e) {
        getPluginInstance().getServer().getScheduler().runTaskAsynchronously(getPluginInstance(), () ->
        {
            ...
            getPluginInstance().writeToLog("[" + getPluginInstance().getDateFormat().format(new Date(System.currentTimeMillis())) + "] "
                    ...
                    + StringUtils.capitalize(e.getEditType().name().toLowerCase().replace("_", " "))
   ...
   /* @EventHandler(ignoreCancelled = true)
    ...
    public Pair<Boolean, Boolean> shopCreationWorks(@NotNull Player player, @NotNull Block block, @NotNull ItemStack itemStack) {
        ...
        if (marketRegion != null) {
            getPluginInstance().getServer().getScheduler().runTaskLater(getPluginInstance(), () -> {
                if (Ref.isOlderThan(11)&&!Ref.isMajorNewerThan(26)) {

...
public class Ref {
    ...
    static String ver = "UKNOWN";
    static int intVer;
    static int intRelease;
    static int major;
    static ServerType type = ServerType.CUSTOM;
        ...
        if (type.isBukkit()) {
            if (serverVersion.indexOf('.') != -1) {
                ...
                if (Ref.ver.split("\\.").length > 2) {
                    Ref.intRelease = ParseUtils.getInt(Ref.ver.split("\\.")[2]);
                }
                Ref.major = ParseUtils.getInt(Ref.ver.split("\\.")[0]);
                return;
            }
            major = -1;
    ...
    public static int majorVersionInt(){
        return Ref.major;
    }
    ...
    public static boolean isMajorNewerThan(int i){
        return Ref.major >= i;
    }

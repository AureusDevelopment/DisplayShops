...
public class MenuListener implements Listener {
    ...
    public void onOpen(InventoryOpenEvent e) {
        ...
        final String invName = INSTANCE.getInventoryName(e.getInventory(), e.getView());
            ...
            if ((invName != null && INSTANCE.matchesAnyMenu(invName))) {
                ...
                if (dp.getSelectedShop() != null) {
                    Bukkit.getScheduler().runTaskLater(INSTANCE, () -> {
                        ...
                        String openInvTitle = INSTANCE.getPlayerTopInventoryName(player);
                        if (openInvTitle != null && INSTANCE.matchesAnyMenu(openInvTitle)) {
                            return;
                        }
                        if (INSTANCE.getVersionUtil().getInventoryType(player.getOpenInventory()) == InventoryType.ANVIL) {
                            return;
                        }
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        if (creationItemName != null && !creationItemName.isEmpty()) {
            ...
            try {
                ...
                for (int i = -1; ++i < blockedInventories.length; ) {
                    if (INSTANCE.getVersionUtil().getInventoryType(player.getOpenInventory()).name().startsWith(blockedInventories[i])) {
        ...
        final String inventoryName = INSTANCE.getInventoryName(e.getClickedInventory(), e.getView());
            ...
            case "confirm": {
                ...
                switch (dataPack.getInteractionType()) {
                    case AMOUNT_BUY_PRICE: {
                        ...
                        double foundBuyPriceMax = (shop.getShopItem() == null ? 0.0 : INSTANCE.getManager().getMaterialMaxPrice(shop.getShopItem(), true));
                        double foundBuyPriceMin = (shop.getShopItem() == null ? 0.0 : INSTANCE.getManager().getMaterialMinPrice(shop.getShopItem(), true));
                        if (amount < (foundBuyPriceMin * shop.getShopItemAmount()) || amount > (foundBuyPriceMax * shop.getShopItemAmount())) {
                            INSTANCE.getManager().sendMessage(player, INSTANCE.getLangConfig().getString("buy-price-limit"));
                            return;
                        }
                        if (amount < shop.getSellPrice(false) && shop.getSellPrice(false) >= 0) {
                            INSTANCE.getManager().sendMessage(player, INSTANCE.getLangConfig().getString("buy-price-less"));
                            return;
                        }
                        ...
                        if (shop.getBuyPrice(false) < 0) shop.setBuyPrice(Math.max(amount, shop.getBuyPrice(false)));
                    ...
                    case AMOUNT_SELL_PRICE: {
                        ...
                        double foundSellPriceMax = shop.getShopItem() == null ? 0 : INSTANCE.getManager().getMaterialMaxPrice(shop.getShopItem(), false);
                        double foundSellPriceMin = shop.getShopItem() == null ? 0 : INSTANCE.getManager().getMaterialMinPrice(shop.getShopItem(), false);
                        if (amount < (foundSellPriceMin * shop.getShopItemAmount()) || amount > (foundSellPriceMax * shop.getShopItemAmount())) {
                            INSTANCE.getManager().sendMessage(player, INSTANCE.getLangConfig().getString("sell-price-limit"));
                            return;
                        }
                        if (amount > shop.getBuyPrice(false) && shop.getBuyPrice(false) >= 0) {
                            String message = INSTANCE.getLangConfig().getString("sell-price-greater");
    ...
    public void playClickSound(@NotNull Player player) {
        DisplayShops.getPluginInstance().getVersionUtil().playSound(player, (Ref.isNewerThan(8) || Ref.isMajorNewerThan(26)) ? (Ref.isNewerThan(20) || Ref.isMajorNewerThan(26)) ? "ui.button.click" : "UI_BUTTON_CLICK" : "CLICK");
    }

    /*public String getInventoryName_V2(Player player){
        return DisplayShops.getPluginInstance().getVersionUtil().getInventoryName()
    }*/



    /*public String getInventoryName(@NotNull Inventory inventory, @NotNull Object inventoryView) {
        if (Ref.isNewerThan(13)) {
            return DisplayShops.getPluginInstance().getVersionUtil().getInventoryName(inventoryView);
        } else {
            try {
    ...
    private void runEconomyCall(@NotNull Player player, @NotNull Shop shop, @NotNull EconomyCallType economyCallType, int unitCount) {
        ...
        if (message != null && !message.equalsIgnoreCase("")) {
            ...
            INSTANCE.getManager().sendMessage(player, message.replace("{item}", (shop.getShopItem().hasItemMeta() && shop.getShopItem().getItemMeta() != null
                            ...
                            : StringUtils.capitalize(shop.getShopItem().getType().name().toLowerCase().replace("_", " ")))
                ...
                if (ownerDP.isTransactionNotify()) {
                    INSTANCE.getManager().sendMessage(ownerPlayer, Objects.requireNonNull(INSTANCE.getLangConfig().getString("shop-" + ecoType + "-owner"))
                            .replace("{item}", (shop.getShopItem().hasItemMeta() && shop.getShopItem().getItemMeta() != null
                                    && shop.getShopItem().getItemMeta().hasDisplayName()) ? shop.getShopItem().getItemMeta().getDisplayName() :
                                    StringUtils.capitalize(shop.getShopItem().getType().name().toLowerCase()
